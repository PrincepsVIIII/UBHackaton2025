...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('role', sa.Enum('elder', 'volunteer', name='emailotp_role_enum'), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column('completed_events_count', sa.Integer(), nullable=False, server_default='0'),
        sa.ForeignKeyConstraint(['user_id'], ['users.id'], ondelete='CASCADE'),
        sa.PrimaryKeyConstraint('user_id')
    )
    ...
    op.create_table(
        ...
        sa.Column('urgency_level', sa.Enum('low', 'medium', 'high', 'critical', name='urgencylevel_enum'), nullable=False),
        sa.Column('weather_factor', sa.Float(), nullable=False, server_default='0.0'),
        sa.Column('status', sa.Enum('open', 'assigned', 'completion_pending_approval', 'completed', 'cancelled', name='requeststatus_enum'), nullable=False, server_default='open'),
        sa.Column('created_at', sa.DateTime(), nullable=False),
